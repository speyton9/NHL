...
class Optimizer:
	...
	def create_indicators(self):
		...
		#Create player ppline indicators so you know which ppline by their team they are on
		for i, ppline in enumerate(self.skaters_df.loc[:, 'ppLine']):
			player_ppline = []
			if int(ppline) == 1:
				player_ppline.extend((1, 0, 0, 0))
			elif int(ppline) == 2:
				player_ppline.extend((0, 1, 0, 0))
			elif int(ppline) == 3:
				player_ppline.extend((0, 0, 1, 0))
			elif int(ppline) == 4:
				player_ppline.extend((0, 0, 0, 1))
			else:
				player_ppline.extend((0, 0, 0, 0))
			player_pplines = []
			for team in teams:
				if self.skaters_df.loc[i, 'team'] == team:
					player_pplines.extend(player_ppline)
				else:
					player_pplines.extend((0, 0, 0, 0))
			self.team_pplines.append(player_pplines)
		self.num_pplines = len(self.team_pplines[0])
